...
public class TxnController {
    ...
    public ResponseEntity<TxnResponse> executeTransaction(@RequestBody TxnRequest request) {
        ...
        if (response.getError() != null && response.getError().equals("NOT_LEADER")) {
            // 重定向到 Leader
            return ResponseEntity.status(301)
                    .header("Location", "http://" + response.getLeaderEndpoint() + "/txn")
                    .body(response);
        }
    ...
    public ResponseEntity<TxnResponse> cas(@RequestBody Map<String, Object> request) {
        ...
        if (response.getError() != null && response.getError().equals("NOT_LEADER")) {
            return ResponseEntity.status(301)
                    .header("Location", "http://" + response.getLeaderEndpoint() + "/txn/cas")
                    .body(response);
        }
    ...
    public ResponseEntity<TxnResponse> acquireLock(@RequestBody Map<String, Object> request) {
        ...
        if (response.getError() != null && response.getError().equals("NOT_LEADER")) {
            return ResponseEntity.status(301)
                    .header("Location", "http://" + response.getLeaderEndpoint() + "/txn/lock")
                    .body(response);
        }
    ...
    public ResponseEntity<TxnResponse> releaseLock(@RequestBody Map<String, Object> request) {
        ...
        if (response.getError() != null && response.getError().equals("NOT_LEADER")) {
            return ResponseEntity.status(301)
                    .header("Location", "http://" + response.getLeaderEndpoint() + "/txn/unlock")
                    .body(response);
        }
